...
has(/arabic:\s*0/, 'brand summaries must include an Arabic-or-mixed counter');
...
has(/status\s*===\s*['"]skipped_arabic_or_mixed['"]/, 'green brand counter must count Arabic or mixed original descriptions');
has(/statusGroup\(state\.status\)\s*===\s*['"]untranslated['"]/, 'red brand counter must count English descriptions ready for translation');
has(/value="arabic_asc"/, 'brand sort must support green count ascending');
has(/value="arabic_desc"/, 'brand sort must support green count descending');
...
has(/brand-count green/, 'brand rows must render the Arabic count in green');

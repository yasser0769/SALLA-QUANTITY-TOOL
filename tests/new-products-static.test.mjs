...
has(/id="brandExportPanel"/, 'page must include a separate new-brand review panel');
has(/id="brandReviewBody"/, 'new-brand review must render editable rows');
...
has(/function\s+updateBrandExportCell/, 'page must persist edits made in brand review');
has(/الصق رابط الشعار هنا/, 'brand review must clearly expose an editable logo URL field');
has(/function\s+translateBrandRows/, 'page must translate brand names through the server route');
has(/function\s+downloadNewBrands/, 'page must export a separate new-brand workbook');
has(/brand_translate/, 'page must request the protected brand translation operation');
has(/brand_health/, 'page must preflight OpenRouter before translating brands');
has(/salla_new_brands_/, 'new-brand download must have a clear filename');
has(/BRAND_EXPORT_HEADERS/, 'page must preserve the exact seven brand export headers');
...
has(/https:\/\/s\.salla\.sa\/settings\/component\/import/, 'page must link to Salla new product import page');
...
notHas(/id="translationProvider"/, 'translation settings must not include a browser provider selector');
notHas(/id="translationApiKey"/, 'translation settings must not expose a direct provider API key input');
...
has(/X-Translation-Access-Token/, 'page must send the same translation access token header as the other translation page');
...
notHas(/https:\/\/openrouter\.ai\/api\/v1\/chat\/completions/, 'OpenRouter endpoint must not be used by the browser page');
notHas(/deepseek\/deepseek-v4-pro/, 'OpenRouter model names must not be used by the new-products translation page');
...
has(/function\s+syncTranslationModelOptions/, 'page must keep DeepSeek model choices in sync');
...
notHas(/response_format\s*:\s*\{\s*type\s*:\s*['"]json_object['"]\s*\}/, 'browser page must leave provider-specific JSON formatting to the API route');
